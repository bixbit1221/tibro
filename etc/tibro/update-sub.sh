...
# --- Конвертация vless:// строк в JSON ---
parse_vless_to_json() {
    local infile="$1"
    local outfile="$2"
    local first=1
    ...
    while IFS= read -r line; do
        ...
        local uuid host port params name transport security sni path_v ws_host service
...
if [ -s "$TMP/remnawave_raw" ]; then
    ...
    parse_vless_to_json "$TMP/remnawave_lines" "$NODES_DIR/remnawave.json"
...
if [ -s "$TMP/happ_raw" ]; then
    ...
    parse_vless_to_json "$TMP/happ_lines" "$NODES_DIR/happ.json"
...
if [ -n "$SUB_PROVIDER3" ]; then
    ...
    if [ -s "$TMP/provider3_raw" ]; then
        ...
        parse_vless_to_json "$TMP/provider3_lines" "$NODES_DIR/provider3.json"
