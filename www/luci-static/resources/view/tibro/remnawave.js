...
@media (max-width: 600px) {
  ...
  td:before { position:absolute; left:6px; width:40%; white-space:nowrap; font-weight:bold; color:#888; content:attr(data-label); }
        ...
        <div class="tibro-wrap">
          <h2>&#128268; <span id="page-label-rw">Remnawave</span> <button class="btn-ping-one" onclick="editLabelRW()" title="Переименовать">&#9998;</button></h2>
          <div class="tibro-card">
          <div class="tibro-card">
            <label style="display:block;margin-bottom:6px;color:#aaa;font-size:12px;">URL подписки:</label>
            <input type="text" id="sub-url-rw" style="width:100%;box-sizing:border-box;padding:8px;border-radius:4px;border:1px solid #333;background:#0d0d1a;color:#eee;font-size:12px;margin-bottom:8px;">
            <button class="tibro-btn btn-connect" onclick="saveSubRW()">&#128190; Сохранить и обновить</button>
            <span id="sub-status-rw" style="margin-left:10px;font-size:12px;color:#888;"></span>
          </div>
            <button class="tibro-btn btn-refresh" onclick="loadNodesRW()">&#8635; Обновить</button>
            <button class="tibro-btn btn-ping-all" onclick="pingNodes()">&#9889; Все пинги</button>
            <div id="nodes-container-rw" style="margin-top:10px"><p style="color:#888">Загрузка...</p></div>
          </div>
        </div>`;

        var PROVIDER = 'remnawave';
        var activeProvider = '';
        var activeIdx = -1;
        var pendingIdx = -1;
        var allNodes = [];
        ...
        function pingOne(i, host, port) {
            var el = document.getElementById('ping-rw-' + i);
            if (el) el.innerHTML = '<span class="ping-wait">&#8987;</span> <button class="btn-ping-one" onclick="pingSingleRW(' + i + ')">&#9889;</button>';
            ...
            .then(function(d) {
                ...
                if (el) el.innerHTML = (pingCache[i] < 1000
                    ...
                    ' <button class="btn-ping-one" onclick="pingSingleRW(' + i + ')">&#9889;</button>';
            }).catch(function() {
                pingCache[i] = 9999;
                if (el) el.innerHTML = '<span class="ping-bad">&#9679; err</span> <button class="btn-ping-one" onclick="pingSingleRW(' + i + ')">&#9889;</button>';
            });
        }
        ...
        function renderNodes() {
            ...
            allNodes.forEach(function(n, i) {
                ...
                var pingBtn = '<button class="btn-ping-one" onclick="pingSingleRW(' + i + ')">&#9889;</button>';
                ...
                var btn = isActive
                    ...
                    : isPending
                        ...
                        : '<button class="tibro-btn btn-connect" onclick="switchNodeRW(' + i + ')">&#9654; Подключить</button>';
                html += '<tr' + rowClass + '>' +
                    '<td data-label="#">' + i + '</td>' +
                    ...
                    '<td data-label="Пинг" id="ping-rw-' + i + '">' + ping + ' ' + pingBtn + '</td>' +
                    '<td data-label="Действие">' + btn + '</td></tr>';
            });
            html += '</table>';
            var el = document.getElementById('nodes-container-rw');
        ...
        window.pingSingleRW = function(i) {
            var n = allNodes[i];
            if (n) pingOne(i, n.host, n.port);
        };

        window.loadNodesRW = function() {
            getApi('').then(function(s) {
                activeProvider = s.provider;
                activeIdx = parseInt(s.node_idx);
                var inp = document.getElementById('sub-url-rw');
                if (inp && !inp.dataset.touched) inp.value = s.sub_remnawave || '';
                var lbl = document.getElementById('page-label-rw');
                if (lbl && s.label_remnawave) lbl.textContent = s.label_remnawave;
        ...
        window.saveSubRW = function() {
            var inp = document.getElementById('sub-url-rw');
            var st = document.getElementById('sub-status-rw');
            if (!inp || !inp.value.trim()) return;
            if (st) st.textContent = 'Сохранение...';
            getApi('action=setsub&provider=remnawave&url=' + encodeURIComponent(inp.value.trim()))
            .then(function(d) {
        ...
        window.editLabelRW = function() {
            var span = document.getElementById('page-label-rw');
            if (!span) return;
            ...
            getApi('action=setlabel&provider=remnawave&label=' + encodeURIComponent(newLabel.trim()))
            .then(function(d) {
                if (d.ok) span.textContent = newLabel.trim();
            });
        };

        window.switchNodeRW = function(idx) {
            pendingIdx = idx;
            renderNodes();
