        ...
        window.loadNodes = function() {
            ...
            fetch('/cgi-bin/tibro-api')
                ...
                .then(s => {
                    ...
                    return fetch('/cgi-bin/tibro-api?action=nodes&provider=happ');
                })
                .then(r => r.json())
                .then(nodes => {
                    if (!nodes.length) {
                    ...
                    nodes.forEach(function(n, i) {
                        var isActive = activeProvider === 'happ' && i === activeIdx;
                        var rowClass = isActive ? ' class="active-row"' : '';
                        var btn = isActive
                            ? '<span style="color:#7fff7f">&#9679; Активна</span>'
                            : '<button class="tibro-btn btn-connect" onclick="switchNode(\'happ\',' + i + ')">&#9654; Подключить</button>';
